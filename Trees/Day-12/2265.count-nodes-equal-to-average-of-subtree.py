...
class Solution:
    def averageOfSubtree(self, root: Optional[TreeNode]) -> int:
        def traverse(root):
            if(not root):
                return [0, 0, 0] # sum, elements, count
            left = traverse(root.right)
            right = traverse(root.left)
            totalElements = left[1] + right[1] + 1
            totalValue = left[0] + right[0] + root.val
            count = left[-1] + right[-1] 
            if((totalValue // totalElements) == root.val):
                count += 1
            return [totalValue, totalElements, count]
