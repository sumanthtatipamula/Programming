...
class Solution:
    def mostProfitablePath(self, edges: List[List[int]], bob: int, amount: List[int]) -> int:
        path, adj, self.result = defaultdict(int), defaultdict(list), -float("Inf")
        for u,v in edges:
            adj[u].append(v)
            adj[v].append(u)
        def generateBobPath(source, parent, timeTaken):
            if(source == 0):
                path[source] = timeTaken
                return True
            for child in adj[source]:
                if(child != parent and generateBobPath(child, source, timeTaken + 1)):
                    path[source] = timeTaken
                    return True
            return False
        generateBobPath(bob, -1, 0)
        """
            Alice < Bob - Alice opens gate
            Alice == Bob - Alice share
            Alice > Bob - Gate is opened
            Alice might not been in bob's path - Alice opens gate
        """
        def findProfitableAlicePath(source, parent, timeTaken, money):
            if(source not in path):
                money += amount[source]
            else:
                if(path[source] > timeTaken):
                    money += amount[source]
                elif(path[source] == timeTaken):
                    money += amount[source] // 2
            if(len(adj[source]) == 1 and adj[source][0] == parent):
                self.result = max(self.result, money)
                return
            for child in adj[source]:
                if(child != parent):
                    findProfitableAlicePath(child, source, timeTaken + 1, money)
        findProfitableAlicePath(0, -1, 0, 0)
        return self.result
