...
class Solution:
    def cloneGraph(self, node: 'Node') -> 'Node':
        if(not node):
            return node
        nodemap, queue, new_root = {}, deque([node]), None
        new_root = Node(node.val)
        nodemap[new_root.val] = new_root
        while(queue):
            node = queue.popleft()
            root = nodemap[node.val]
            for child in node.neighbors:
                if(child.val in nodemap):
                     root.neighbors.append(nodemap[child.val])
                     continue
                new_child = Node(child.val)
                root.neighbors.append(new_child)
                nodemap[child.val] = new_child
                queue.append(child)
        return new_root
