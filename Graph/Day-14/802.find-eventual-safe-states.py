...
class Solution:
    def eventualSafeNodes(self, graph: List[List[int]]) -> List[int]:
        vis, isTerminal, result = [False] * len(graph), [False] * len(graph), [] 
        def dfs(source):
            if(vis[source]):
                return isTerminal[source]
            vis[source] = True
            terminal = True
            for child in graph[source]:
                terminal = terminal and dfs(child)
            isTerminal[source] = terminal
            return terminal
        for i in range(len(graph)):
            if(not vis[i]):
                dfs(i)
        return [i for i in range(len(isTerminal)) if(isTerminal[i])]
