...
class Solution:
    def isSubTree(self, root1, root2):
        serialset = set()
        def serialize(root, addToSet = False):
            if(not root):
                return '#'
            serial_id = str(root.data) + ',' + serialize(root.left, addToSet) + ',' + serialize(root.right, addToSet)
            if(addToSet):
                serialset.add(serial_id)
            return serial_id
        serialize(root1, True)
        return serialize(root2) in serialset
