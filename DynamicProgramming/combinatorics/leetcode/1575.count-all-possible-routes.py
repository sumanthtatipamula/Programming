...
class Solution:
    def countRoutes(self, locations: List[int], start: int, finish: int, fuel: int) -> int:
        mod = 10 ** 9 + 7
        mem = {}
        def findWays(currentPos, fuelRemaining):
            if(fuelRemaining <= 0):
                return 0 if(currentPos != finish) else 1
            ways = 1 if(currentPos == finish) else 0
            if((currentPos, fuelRemaining) in mem):
                return mem[(currentPos, fuelRemaining)]
            for i in range(0, len(locations)):
                if(i != currentPos and fuelRemaining >= abs(locations[currentPos] - locations[i])):
                    ways = (ways + findWays(i, fuelRemaining - abs(locations[currentPos] - locations[i]))) % mod
            mem[(currentPos, fuelRemaining)] = ways
            return mem[(currentPos, fuelRemaining)]
