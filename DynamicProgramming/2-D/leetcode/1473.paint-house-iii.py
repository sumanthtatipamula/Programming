...
class Solution:
    def minCost(self, houses: List[int], cost: List[List[int]], m: int, n: int, target: int) -> int:
        @lru_cache(None)
        def findCost(index, k, prev_color):
            if(index == m and target == k):
                return 0
            if(index >= m or k > target):
                return float("Inf")
            if(houses[index]):
                return findCost(index + 1, k + (prev_color != houses[index]), houses[index])
            else:
                min_cost = float("Inf")
                for j in range(1, n + 1):
                    min_cost = min(min_cost, findCost(index + 1, k + (j != prev_color), j) + cost[index][j - 1])
                return min_cost
        result = findCost(0, 0, None)
